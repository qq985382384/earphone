...
class Goods(models.Model):  #商品
    ...
    types = models.ForeignKey(Types,on_delete = models.DO_NOTHING) #一个分类会有多个商品
    seller = models.ForeignKey(Seller, on_delete=models.DO_NOTHING) #一家店铺会有多个商品

class Image(models.Model):  #商品展示托盘
    img_path = models.ImageField(upload_to = "image")
    img_label = models.CharField(max_length = 32)
    goods = models.ForeignKey(Goods, on_delete=models.DO_NOTHING)  # 一个商品多张图片
